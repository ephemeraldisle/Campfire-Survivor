...
var _total_loading_progress : float = 0.0 :
	set(value):
		...
		%Percent.text = "%d%%" % (_total_loading_progress * 100)
...
func _ready() -> void:
	GameEvents.loaded.connect(kll_me) 
	GameEvents.main_menu_loaded.connect(kll_me)
	GameEvents.loadin.emit()
...
func _process(_delta):
	if not _changing_to_next_scene:
		...
		%Title.text = "Reticulating splines..."
		_total_loading_progress = (float(GameState.loaded_terrain) / float(GameState.terrain_total) )
		#extra_time += _delta
